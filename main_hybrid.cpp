/*
 *  nonbinary spa.cpp v. 1.3.0	(hybrid version)
 *  							
...
int main(int argc, char * argv[])
{
	...
	unsigned int spa_iterations = 50;							// spa iterations
	unsigned int test_cases = atoi(argv[2]);					// simulations an each error case.
	
	unsigned int rp_period = 6;
	unsigned int ef_period = 30;
	...
	*/
	...
	double temp_prior[_N][_error_types];
        	...
        	for(int test_idx = 0; test_idx < test_cases; ++test_idx)
        	{
				...
				*/
				...
				if(process_id == process_root)
				{
					...
					depolarizing_channel_nonbinary(error, error_weight, _error_types, _N);
...
// update prior		
					...
					{
						if(spa_idx%ef_period == 0 && spa_idx)	// enhanced feedback
						{
							for(int j=0; j<_N; ++j)
							{
								prior[j][0] = 1-error_prob[sim_idx];
								memset(&prior[j][1], prior_each, sizeof(double)*(_error_types-1));
							}
							...
							for(int i=0; i<_M; ++i) 
							{
								if(incompatible[i]) frustrated_checks[temp_idx++] = i;
							}
							...
							CommutativityTableB(_gf_tableB, table, _row_op[random_frustrated][random_qubit_idx]);		
							...
							for(int j=0; j<_N; ++j)
							{
								memcpy(temp_prior[j], prior[j], sizeof(double)<<_2_gf_extension); 
							}
						}
						else if(spa_idx%rp_period == 0 && spa_idx)	// random perturbation
						{	
							srand(time(NULL));
							
							for(int j=0; j<_N; ++j)
							{
								memcpy(prior[j], temp_prior[j], sizeof(double)<<_2_gf_extension); 
							}
    		        ...
    		        if(spa_idx && spa_idx%rp_period==0) 
    		        	MPI_Bcast(prior, _NE, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    	        }
        	}
        }
        else
